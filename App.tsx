...
interface StoredState {
  selectedTemplateId: string;                                                   imageConfig: ImageConfig;
  theme: ThemeId;
}                                                                             
const App: React.FC = () => {
  // ---------- ESTADOS ----------
  ...
  useEffect(() => {
    const stored = localStorage.getItem(STORAGE_KEY);                             if (stored) {
      try {
        const parsed = JSON.parse(stored) as StoredState;
  ...
  const handleScaleChange = useCallback((newScale: number) => {
    setImageConfig(prev => {
      const oldScale = prev.scale;                                                  const focusX = prev.focusX;
      ...
      // La fórmula correcta para mantener el punto de foco en su lugar es:         // nuevaPosición = posiciónOriginalDelFocoEnPantalla - nuevaEscala * posiciónOriginalDelFocoEnImagen
  ...
  const themeStyles = {
    ...
    bgTertiary: currentTheme.bgTertiary,                                          textPrimary: currentTheme.textPrimary,
    textSecondary: currentTheme.textSecondary,
    border: currentTheme.border,
  };
                                                                                return (
    <div                                                                            className="min-h-screen p-4 md:p-6 lg:p-10 font-sans transition-colors duration-300"
      ...
      <header className="max-w-7xl mx-auto mb-8 lg:mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 lg:gap-6">
        <div className="flex items-center gap-4">                                       <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30">
            <Smartphone className="text-white" size={24} />
          </div>
          <div>
            <h1 className="text-2xl lg:text-3xl font-black tracking-tight">
              <span className="text-blue-500">SUBLIME</span>
              <span style={{ color: themeStyles.textPrimary }}>TAL</span>                   <span className="text-blue-500 text-lg ml-1">PRO</span>
            </h1>
        ...
        <div className="flex flex-wrap gap-3">
          {/* Botón de tema */}                                                         <button
            onClick={handleThemeChange}                                                   className="p-2.5 rounded-full hover:bg-opacity-20 transition-colors flex items-center gap-2"                                                                style={{ backgroundColor: theme === 'dark' ? '#262626' : '#e5e5e5', color: themeStyles.textPrimary }}
      ...
      <main className="max-w-7xl mx-auto grid gap-6 lg:gap-10" style={{ gridTemplateColumns: isPanelOpen ? '340px 1fr' : '0px 1fr' }}>                              {/* ---------- PANEL LATERAL COLAPSABLE ---------- */}
        <div
          ...
          {isPanelOpen && (
            <div className="space-y-4 lg:space-y-6 w-[340px]">
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto pr-2 scrollbar-thin">                                                                    {DEVICE_TEMPLATES.map(t => (
                    <DeviceButton                                                                   key={t.id}
                      device={t}
              ...
              >
                <h3 className="text-xs font-bold mb-4 uppercase tracking-widest flex items-center gap-2" style={{ color: themeStyles.textSecondary }}>
                  <Settings2 size={16} /> Ajuste de Imagen                                    </h3>

                {image ? (
                  <div className="space-y-5">
                    {/* Aviso de arrastre obligatorio */}
                    {!hasUserMovedImage && (
                      <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded-xl flex items-center gap-2">                                                             <Move className="text-blue-500 shrink-0" size={16} />                         <p className="text-xs text-blue-400">
                          Arrastra la imagen para posicionarla en la placa
                        </p>
                      </div>
                    )}

                    {/* Control de escala */}                                                     <SliderControl
                      label="Escala"
                      ...
                      theme={theme}
                    />                                                        
                    {/* Botones de Voltear (Flip) */}                                             <div className="space-y-2">
                      <label className="text-xs font-medium flex items-center gap-2" style={{ color: themeStyles.textSecondary }}>                                                  <FlipHorizontal size={14} /> Voltear
                      </label>                                                                      <div className="flex gap-2">
                        <button
                          onClick={handleFlipHorizontal}
                          className="flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2"                                           style={{
                            backgroundColor: imageConfig.flipH ? '#3b82f6' : themeStyles.bgTertiary,                                                                                    color: imageConfig.flipH ? '#ffffff' : themeStyles.textPrimary
                          }}
                        >
                        ...
                        >                                                                               <FlipVertical size={16} />
                          Vertical
                                                                                                  ...
                                                                                                  {/* Reset y quitar */}
                    <div className="flex gap-2 pt-2 border-t" style={{ borderColor: themeStyles.border }}>                                                                        <button
                        onClick={handleResetTransforms}                                               className="flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2"
                    ...
                    <div className="pt-3 border-t" style={{ borderColor: themeStyles.border }}>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">                                <div className="p-2 rounded-lg" style={{ backgroundColor: themeStyles.bgTertiary }}>
                          <span style={{ color: themeStyles.textSecondary }}>Ancho</span>
                          <p className="font-mono">{image.width}px</p>                                </div>
                        <div className="p-2 rounded-lg" style={{ backgroundColor: themeStyles.bgTertiary }}>
                          <span style={{ color: themeStyles.textSecondary }}>Alto</span>                                                                                              <p className="font-mono">{image.height}px</p>                               </div>                                                                      </div>
                    </div>
                  </div>
                ) : (
              ...
              {/* Nota informativa */}                                                      <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl flex gap-3">
                <AlertCircle className="text-amber-500 shrink-0 mt-0.5" size={18} />                                                                                        <p className="text-xs leading-relaxed" style={{ color: theme === 'dark' ? '#fde68a' : '#b45309' }}>                                                           <strong className="text-amber-400">Nota:</strong> El área negra representa el hueco de la cámara. Asegúrate de que el contenido principal (rostros, logos) no quede bajo esa zona.                                                      </p>
              </div>
            </div>
          )}
        </div>                                                                                                                                                      {/* ---------- BOTÓN PARA COLAPSAR PANEL ---------- */}
        <button
          onClick={togglePanel}
          ...
          {isPanelOpen ? <PanelLeftClose size={20} /> : <PanelLeft size={20} />}                                                                                    </button>
                                                                                      {/* ---------- VISOR PRINCIPAL ---------- */}
        <div
          ...
          style={{                                                                        backgroundColor: themeStyles.bgSecondary,
        ...
        >
          ...
          <DevicePreview
            ...
            image={image}                                                                 imageConfig={imageConfig}
          ...
          <div className="mt-6 lg:mt-8 text-center">                                      <p className="text-xs font-mono uppercase tracking-wider mb-3" style={{ color: themeStyles.textSecondary }}>                                                  Vista Previa de Placa de Metal                                              </p>                                                                          <div className="flex flex-wrap gap-2 justify-center">
              ...
              <span className="px-3 py-1.5 rounded-full text-[10px] font-mono" style={{ backgroundColor: themeStyles.bgTertiary, color: themeStyles.textSecondary }}>                                                                                     H: {selectedTemplate.height}mm
              </span>
              {image && (
      ...
      {/* ---------- FOOTER ---------- */}
      <footer className="max-w-7xl mx-auto mt-10 pt-6 text-center" style={{ borderColor: themeStyles.border, borderTopWidth: 1, borderStyle: 'solid' }}>            <p className="text-xs" style={{ color: themeStyles.textSecondary }}>            SUBLIMETAL PRO © 2024 — Simulador de impresión para sublimación en metal
        </p>                                                                        </footer>
    </div>
  );
};
